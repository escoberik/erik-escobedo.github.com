...
(function (root, factory) {
	...
	Raphael.fn.freeTransform = function(subject, options, callback) {
		...
		var
			...
			;
		var w = subject.getBBox().width;
		var h = subject.getBBox().height;
		...
		ft.updateHandles = function() {
			...
			ft.axes.map(function(axis) {
				if ( ft.handles[axis] ) {
					...
					//Rabie: Don't apply boundary limit on handle
					// Keep handle within boundaries
					if ( ft.opts.boundary  && false) {
					...
					var rc = {x: ft.attrs.center.x + ft.attrs.translate.x, y: ft.attrs.center.y + ft.attrs.translate.y};
					var hc = {x: ft.handles[axis].disc.attrs.cx, y: ft.handles[axis].disc.attrs.cy};
					var dir = {x: rc.x - hc.x, y: rc.y - hc.y};
					var dist = dist = Math.sqrt( (dir.x * dir.x)+(dir.y * dir.y) );
					dir.x *= 1.0/dist;
					dir.y *= 1.0/dist;
					var endPoint;
					if(ft.attrs.rotate%90 == 0)
					{
						w = subject.getBBox().width;
						h = subject.getBBox().height;
						endPoint = { x: hc.x + (dir.x * (dist - w/2)), y: hc.y + (dir.y * (dist - h/2)) };
					}
					else
					{
						endPoint = {x: hc.x + (dir.x * (dist - w/3)), y: hc.y + (dir.y * (dist - h/3)) };
					}
					
					ft.handles[axis].line1.toFront().attr({
						path: [ 
						[ 'M',  hc.x, 
								hc.y ], 
						[ 'L',  endPoint.x, 
			...
			//Rabie: Change cx, cy to x,y cause I changed the circle into a rect handle. Also added rotation
			if ( ft.handles.center ) {
				ft.handles.center.disc.toFront().attr({
					//cx: ft.attrs.center.x + ft.attrs.translate.x,
					//cy: ft.attrs.center.y + ft.attrs.translate.y
					x: bbox.x + ft.attrs.translate.x,
					y: bbox.y + ft.attrs.translate.y
				})
				.transform('R' + ft.attrs.rotate);
				ft.handles.center.disc.toBack()
			}
		...
		ft.showHandles = function() {
			...
			ft.axes.map(function(axis) {
				...
				//Rabie: Actually visible line, that is not drawn from the center. 
				ft.handles[axis].line1 = paper
					.path([ 'M', ft.attrs.center.x, ft.attrs.center.y + bbox.height/2 ])
					.attr({
						stroke: ft.opts.attrs.stroke,
						"stroke-width" : 3,
						//'stroke-dasharray': '- ',
				...
				ft.handles[axis].disc = paper
					...
					.attr(ft.opts.attrs)
					;
				ft.handles[axis].disc.attr({stroke:0.5});
			});
			...
			if ( ft.opts.drag.indexOf('center') !== -1 ) {
				...
				ft.handles.center.disc = paper
					//.circle(ft.attrs.center.x, ft.attrs.center.y, bbox.width/2)//ft.opts.size.center)
					.rect(bbox.x, bbox.y, bbox.width, bbox.height)
					.attr(ft.opts.attrs)
					;
				ft.handles.center.disc.attr({stroke:0.5, fill:'rgba(0,0,0,0)'});
			}
